...
try {
    ...
    if (!$reserva) {
        $_SESSION['error'] = 'La reserva no existe o no tienes permiso para aceptarla.';
        header('Location: ../vistas_usuarios/perfil_autonomo.php');
        exit();
    }

    // Actualizar el estado de confirmación a 'aceptada'
    $stmt = $pdo->prepare("UPDATE reservas SET estado_confirmacion = 'aceptada' WHERE id_reserva = ?");
    ...
    $_SESSION['mensaje'] = 'Has aceptado la reserva de ' . $info_cliente['nombre'] . ' ' . $info_cliente['apellido'] . ' para el servicio ' . $info_cliente['nombre_servicio'];
